"""
Setup script to add product categories
Run this with: python manage.py shell -c "exec(open('setup_categories.py').read())"
"""
import os
import django
...
# Check current count
current_count = ProductCategory.objects.count()
print(f"\nCurrent categories in database: {current_count}")
...
print(f"\nAdding {len(categories_data)} categories...")
print("-" * 60)
...
print(f"   Total in database: {ProductCategory.objects.count()}")
print("=" * 60)
