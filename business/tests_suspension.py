# Purpose: Tests that a 'suspended' business is read-only for the seller and closed to machines.
# Used by: python manage.py test business.tests_suspension
# Notes: 'pending' and 'inactive' must stay untouched — the regression guards below are the
#        ones protecting new signups, since 'pending' is the default for every registration.
...
class ActiveBusinessNotGatedTest(TestCase):
    """Regression guard: pending is the default for every new signup."""

    def test_pending_and_inactive_reach_the_view(self):
        for idx, status in [(9111, 'pending'), (9112, 'inactive'), (9113, 'active')]:
