...
def adjust_warehouse_inventory_on_status_change(sender, instance, created, **kwargs):
    """
    Automatically adjust warehouse inventory based on delivery task status changes.

    Flow:
    - picked_up status → Decrease inventory (items leaving warehouse)
    - failed status → Increase inventory (items back to warehouse)
    - delivered status → Confirm decrease (final confirmation)

    Only processes if:
    - Order has fulfillment service enabled
    - Order items have product references
    - Products exist in warehouse inventory
    """
    # Skip if this is a new task (no status change yet)
    if created:
        return
    
    # Check if order has fulfillment service enabled
    ...
    logger.info(f"Delivery task {instance.dl_task_number} status changed: {old_status} → {new_status}")
    ...
    try:
        ...
        if not order_items.exists():
            logger.debug(f"No product-linked order items for order {instance.order.order_number}")
            return
        
        # Get warehouse link for this business
        warehouse_link = warehouse_models.SellerWarehouseLink.objects.filter(
            business=business,
            is_active=True,
            is_default=True
        ).select_related('warehouse', 'default_location').first()
        
        if not warehouse_link:
            logger.warning(f"No active warehouse link found for business {business.business_name}")
            return
        
        warehouse = warehouse_link.warehouse

        # Process inventory adjustments based on status change
        with transaction.atomic():
            for order_item in order_items:
                product = order_item.product
                quantity = order_item.quantity

                # Get stock level for this product at ANY location in the warehouse
                # We use location=None to get/create warehouse-level stock (not location-specific)
                stock_level, created = warehouse_models.StockLevel.objects.select_for_update().get_or_create(
                    warehouse=warehouse,
                    product=product,
                    location=None,  # Warehouse-level stock, not tied to specific storage location
                    defaults={
                        'quantity_on_hand': 0,
                        'quantity_reserved': 0,
                        'reorder_point': 10,
                ...
                # Determine adjustment based on status transition
                adjustment_type = None
                quantity_change = 0
                
                if new_status == 'picked_up' and old_status in ['for_review', 'pending', 'assigned', 'accepted']:
                    # Items picked up from warehouse - decrease inventory
                    adjustment_type = 'ship'
                    quantity_change = -quantity
                    stock_level.quantity_on_hand = max(0, stock_level.quantity_on_hand - quantity)

                elif new_status == 'failed' and old_status in ['picked_up', 'start_ride', 'out_for_delivery', 'in_transit', 'contacted', 'non_reachable']:
                    # Items failed delivery - return to warehouse inventory
                    adjustment_type = 'return'
                    quantity_change = quantity
                    stock_level.quantity_on_hand += quantity

                elif new_status == 'delivered' and old_status in ['picked_up', 'start_ride', 'out_for_delivery', 'in_transit', 'contacted']:
                    # Items delivered successfully - confirm decrease (no change needed, already decreased on pickup)
                    adjustment_type = 'confirm_ship'  # Log for audit trail
                    quantity_change = 0  # Already adjusted on pickup
                
                # Save stock level changes
                if quantity_change != 0:
                    stock_level.save()
                    logger.info(
                        f"Stock adjusted for {product.name} (SKU: {product.sku}): "
                        f"{quantity_change:+d} units → New stock: {stock_level.quantity_on_hand}"
                    )
                ...
                if adjustment_type:
                    warehouse_models.InventoryTransaction.objects.create(
                        warehouse=warehouse,
                        product=product,
                        location=None,  # Warehouse-level transaction
                        transaction_type=adjustment_type,
                        quantity=abs(quantity),
                        reference_type='delivery_task',
                        reference_id=str(instance.id),
                        notes=f"Auto-adjusted from delivery status: {old_status} → {new_status} (Order: {instance.order.order_number})"
                    )
        
        logger.info(f"Successfully adjusted warehouse inventory for order {instance.order.order_number}")
        
    except Exception as e:
        logger.error(f"Error adjusting warehouse inventory for task {instance.dl_task_number}: {e}", exc_info=True)


@receiver(pre_save, sender=DeliveryTask)
def delivery_task_pre_save_receiver(sender, instance, **kwargs):
    """Store old delivery task status for change detection."""
    if instance.pk:
        try:
            old_instance = DeliveryTask.objects.get(pk=instance.pk)
            instance._old_dl_task_status = old_instance.dl_task_status
