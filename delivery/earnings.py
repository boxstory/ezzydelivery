...
class RateCard:
    ...
    __slots__ = ('normal_fee', 'hub_fee', 'pick_and_drop_percent', 'source', 'rate_id')

    def __init__(self, normal_fee, hub_fee, pick_and_drop_percent,
                 source='fallback', rate_id=None):
...
FALLBACK_CARD = RateCard(
    FALLBACK_NORMAL_FEE, FALLBACK_HUB_FEE, FALLBACK_PND_PERCENT, source='fallback')
...
def _card_branches(row, extra=None):
    """The three fee branches for one card: hub leg, pick & drop, everything else."""
    scope = _window(row)
